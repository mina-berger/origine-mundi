/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package origine_mundi.opus24;

import com.mina.util.Doubles;
import java.util.ArrayList;
import la.clamor.Cinctum;
import la.clamor.Punctum;
import la.clamor.Vel;
import la.clamor.forma.CadentesFormae;
import la.clamor.forma.Chorus;
import la.clamor.forma.Compressor;
import la.clamor.forma.Distortion;
import la.clamor.forma.Equalizer;
import la.clamor.forma.IIRFilter;
import la.clamor.opus.Mensa;
import la.clamor.opus.Taleae;
import origine_mundi.archive.ArchiveLudior;

/**
 *
 * @author hiyamamina
 */
public class Opus024 extends Mensa {

    public Opus024() {
        super(true, true);
    }

    @Override
    protected void ponoComitis(ArrayList<Taleae.Comes> comites) {
    }

    static double tempo = 94;

    @Override
    protected void ponoRapidi(ArrayList<Taleae.Rapidus> rapidi) {
        rapidi.add(new Taleae.Rapidus(0, 0, tempo, false));
    }

    @Override
    protected void creo() {
        ponoInstrument(0, new Punctum(1.5), new Cinctum(), new ArchiveLudior("mu500r", "clgt", 500), new CadentesFormae(
                //new IIRFilter(500, 10000, true),
                new Compressor(new Punctum(0.2), new Punctum(0.08)),
                new IIRFilter(8000, true),

                new Equalizer(0., 1., 1., 0., -0.5, -0.5, -1., -1., 1., 1.)
                //new Distortion(new Punctum(0.5), new Punctum(2))
                //new Chorus(new Punctum(0.3), new Punctum(6), new Punctum(0.5), new Punctum(0.5))
        ));
        
        for(int i = 0;i < 4;i++){
            Doubles notes = i % 2 == 0?new Doubles(50, 57, 61, 66, 69):new Doubles(45, 57, 60, 64, 71);
            ludo(0, i, 0.0, 0.2, notes, new Vel(1));
            //ludo(0, i, 0.25, 0.2, notes, new Vel(1));
            //ludo(0, i, 1.0, 0.01, notes, new Vel(1));
            //ludo(0, i, 1.25, 0.01, notes, new Vel(1));
            ludo(0, i, 1.5, 2.2, notes, new Vel(1));
            //ludo(0, i, 1.75, 0.2, notes, new Vel(1));
            //ludo(0, i, 3.0, 0.9, notes, new Vel(1));
        }
    }

    @Override
    public void initio() {
    }

    @Override
    protected void anteFacio() {
    }

}
