...
public class Parte027BP extends ParteTaleae {
    ...
    protected void creo(int t) {
        ...
        switch (t % 9) {
            ...
            case 4:
                ludoClaves(t,
                        (int i) -> new Doubles(0, 0.75, 1.5, 2.75, 3.5).get(i),
                        (int i) -> new Doubles(0.7, 0.7, 1.1, 0.6, 0.45).get(i),
            ...
            case 7:
                ludoTriplet(t,
                        ...
                        (int i) -> new Doubles(62, 65, 63, 67, 65, 68, 67, 70, 68, 72, 70).get(i),
                        (int i) -> new Vel(1), 11);
                break;
            case 8:
                ludoTriplet(t,
                        (int i) -> new Doubles(0, 0.33, 0.66, 1, 1.33, 1.66).get(i),
                        (int i) -> new Doubles(0.3, 0.3, 0.3, 0.3, 0.3, 0.3).get(i),
                        (int i) -> new Doubles(74, 72, 75, 74, 77, 75).get(i),
                        (int i) -> new Vel(1), 6);
                ludoClaves(t,
                        (int i) -> new Doubles(2, 2.5).get(i),
                        (int i) -> new Doubles(0.2, 1.2).get(i),
                        (int i) -> new Notes(new Doubles(79, 80).get(i), intervals),
                        (int i) -> new Vel(1), 2);
                break;
        }
    }
    protected void ludoTriplet(int talea, Mutans<Double> repenso, Mutans<Double> diutirnitas, Mutans<Double> clavis, Mutans<Vel> vel, int length) {
        for (int i = 0; i < length; i++) {
            double _diu = diutirnitas.capio(i) / 3.;
            double _rep = 0.11;
            double rep = repenso.capio(i);
            ludo(talea, rep,             _diu * 2, clavis.capio(i),      vel.capio(i));
            ludo(talea, rep + _rep,      _diu,     clavis.capio(i) + 12, vel.capio(i).multiplico(0.8));
            ludo(talea, rep + _rep * 2., _diu,     clavis.capio(i),      vel.capio(i).multiplico(0.8));
        }
    }
